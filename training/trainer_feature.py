...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder1"] = networks.ResnetEncoder_low(
            self.opt.num_layers, self.opt.weights_init == "pretrained")
        self.models["encoder1"].to(self.device)
        self.parameters_to_train += list(self.models["encoder1"].parameters())
    ...
    def run_epoch(self):
        ...
        print("Training")
        self.set_train()
        #if self.epoch >=20 and self.epoch <30:
           #print("15 Frames per second")
           #self.opt.frame_ids=[0,-5,5]
        #if self.epoch >=30:
           #print("20 Frames per second")
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            #print(feature_concat.size())


            outputs = self.models["depth"](features[0])
        else:
            # Otherwise, we only feed the image with frame_id 0 through the depth encoder
            #all_color_aug = (torch.unsqueeze(inputs[("color_aug", 0, 0)],2) )
            features1 = self.models["encoder"](inputs["color_aug", 0, 0])

            features2 = self.models["encoder"](inputs["color_aug", 0, 0])
            ...
            #print(feature_concat.size())
            outputs = self.models["depth"](feature_concat)

        if self.opt.predictive_mask:
            outputs["predictive_mask"] = self.models["predictive_mask"](features)

        if self.use_pose_net:
            outputs.update(self.predict_poses(inputs, features1))
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if combined.shape[1] == 1:
                ...
                #occlusion, idxs_occ = torch.max(combined, dim=1)

            if not self.opt.disable_automasking:
                outputs["identity_selection/{}".format(scale)] = (
                    idxs > identity_reprojection_loss.shape[1] - 1).float()
                #outputs["identity_selection2/{}".format(scale)] = (
                    #idxs_occ > identity_reprojection_loss.shape[1] - 1).float()
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                if self.opt.predictive_mask:
                    ...
                    #writer.add_image(
                        #"occlusion_{}/{}".format(s, j),
                        #outputs["identity_selection2/{}".format(s)][j][None, ...], self.step)
